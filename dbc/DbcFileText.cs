...
namespace axp.Dbc
{
    public partial class DbcFile
    {
        ...
        private static async Task<string> ReadStringAsync(Stream stream, uint limit)
        {
            var textData = new byte[limit];
            var readLength = 0;
            int rCount;
        ...
        private static async Task<DbcFile> ReadTextAsync(Stream stream, uint limit, Encoding encoding)
        {
            var textData = new byte[limit];
            var readLength = 0;
            int rCount;
            ...
            var fileContent = encoding.GetString(textData);
            //
            List<DbcFieldType> fieldTypes = new();
            List<string>? fieldNames;
            SortedDictionary<int, List<DbcField>> dataMap = new();
        ...
        private static async Task LoadTextFieldTypesAsync(StringReader reader, List<DbcFieldType> fieldTypes)
        {
            ...
            {
                throw new Exception("Lỗi đọc dòng dữ liệu đầu tiên của dbc");
            }
            ...
            foreach (var str in fieldTypeStrs)
            {
                ...
                {
                    throw new Exception("Loại trường dữ liệu không xác định " + str);
                }
            }
            if (fieldTypes.Count == 0)
            {
                throw new Exception("Không có trưỡng dữ liệu hợp lệ nào được đọc");
            }
        }
